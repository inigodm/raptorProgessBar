...
import static com.intellij.util.ui.UIUtilities.drawString;

public class RaptorProgressBarUi extends BasicProgressBarUI {
    private volatile int offset = 0;
    private volatile int offset2 = 0;
    private volatile int velocity = 1;
    private final Color green2 = new Color(90,115, 90);

    private final Color green3 = new Color(190,245, 190);
    private final Color[] colors = new Color[]{green2, green2};
    private final float[] fractions;
    private int iconIndex = 0;

    public RaptorProgressBarUi() {
        float division = 1f / colors.length;
        fractions = new float[colors.length];
        for (int i = 0; i< colors.length; i++) {
            fractions[i] = division * (i + 1);
        }
    }
    ...
    public static ComponentUI createUI(JComponent c) {
        ...
        return new RaptorProgressBarUi();
    }
    ...
    protected void paintIndeterminate(Graphics g2d, JComponent c) {
        ...
        Insets b = progressBar.getInsets(); // area for borderm
        ...
        LinearGradientPaint baseRainbowPaint = new LinearGradientPaint(0, JBUIScale.scale(2), 0, h - JBUI.scale(6),
                fractions,
                colors);
        ...
        Icon scaledIcon = velocity > 0
                ? ((ScalableIcon) RaptorIcons.CAT_ICON[iconIndex])
                : ((ScalableIcon) RaptorIcons.RCAT_ICON[iconIndex]) ;
        if (iconIndex == 0) {
            iconIndex = 1;
        }
        else {
    ...
    protected void paintDeterminate(Graphics g, JComponent c) {
        ...
        g2.setPaint(new LinearGradientPaint(0, JBUIScale.scale(2), 0, h - JBUIScale.scale(6),
                fractions,
                colors));
        ...
        g2.translate(0, -(c.getHeight() - h)/2);
        RaptorIcons.CAT_ICON[iconIndex].paintIcon(progressBar, g2, amountFull - JBUIScale.scale(10), -JBUIScale.scale(0));
        if (iconIndex == 0) {
            iconIndex = 1;
        }
        else {
    ...
    private void paintString(Graphics g, int x, int y, int w, int h, int fillStart, int amountFull) {
        ...
        boolean isHorizontal = progressBar.getOrientation() == SwingConstants.HORIZONTAL;
        Graphics2D g2 = (Graphics2D)g;
        ...
        g2.setFont(isHorizontal
                ? progressBar.getFont()
                : progressBar.getFont().deriveFont(AffineTransform.getRotateInstance(Math.PI/2)));
        drawString(progressBar, g2, progressString, renderLocation.x, renderLocation.y);
